...
│   │   ├── database_logic.py   # Логика записи метаданных в SQLite
...
│   │   ├── database_tab.py     # UI конфигурации базы данных
...
│
├── cookies.txt                # Cookies сайта Х (для скачивания видео с возрастными ограничениями)
...
*   **Скачивание 18+ контента с X (Twitter):** Для скачивания видео с возрастным ограничением из X (бывший Twitter) программе требуется токен вашей авторизации. Его нужно поместить в файл `cookies.txt`, который должен находиться в главной папке программы (там же, где `README.md`, `requirements.txt` и так далее).
    **Как получить токен:**
    1. Откройте X (Twitter) в браузере и авторизуйтесь.
    2. Нажмите `F12` (или через меню браузера: Дополнительные инструменты -> Инструменты разработчика).
    3. В верхней панели появившегося окна найдите вкладку **Application** (Приложение). Если её нет, нажмите на стрелочку `>>`.
    4. В левом меню найдите раздел **Storage** (Хранилище), разверните **Cookies** и кликните на `https://x.com`.
    5. В таблице справа найдите строку, где в столбце **Name** указано `auth_token`.
    6. Скопируйте значение этого токена и вставьте его в шаблон файла `cookies.txt`, заменив `YOUR_TOKEN`.
    *Примечание: При первой попытке скачивания может выдать ошибку, так как файл только формируется и считывается через токен, но при следующих запусках всё будет работать без ошибок.*
...
*   **Предпросмотр:** Встроенный плеер позволяет прослушать и просмотреть выделенный участок перед экспортом.
...
### 6. Database (База данных SQLite)
Опциональная вкладка, созданная специально для каталогизации ваших загрузок с YouTube. Если она включена, программа будет извлекать метаданные из скачиваемого ролика и автоматически заносить их в файл базы данных `.db`.

*   **Динамический выбор данных:** Выбирайте только то, что хотите сохранить. Доступно более 20 полей: Название, Прямая ссылка на канал, Просмотры, Лайки, Теги, Имя файла, Итоговый вес файла, а также Абсолютный и Относительный пути к файлу на вашем диске.
*   **Настройка сортировки:** Меняйте порядок столбцов в БД прямо в интерфейсе. Если вы добавите новую галочку к уже существующей базе данных, программа автоматически выполнит команду `ALTER TABLE`, не повредив старые записи.
*   **Безопасность:**
    *   **Защита от дубликатов:** Специальная галочка не даст записать один и тот же `video_url` в таблицу дважды.
    *   **Авто-бэкапы:** При первой записи за день программа автоматически создаст резервную копию базы данных в папке `backups/`.
*   **Управление базой:** Для удобного просмотра и редактирования сгенерированных `.db` файлов мы настоятельно рекомендуем использовать бесплатную программу **[DB Browser for SQLite](https://sqlitebrowser.org/dl/)** ([GitHub Link](https://github.com/sqlitebrowser/sqlitebrowser)).
